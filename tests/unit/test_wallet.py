...
def test_default_initial_amount(empty_wallet):
    ...
    assert empty_wallet.balance == 0.0
...
def test_wallet_add_cash(wallet):
    ...
    amount_to_add = 32.55
    expected_balance = wallet.balance + amount_to_add

    wallet.add_cash(amount_to_add)
...
def test_wallet_spend_cash(wallet):
    ...
    amount_to_spend = 15.3
    expected_balance = wallet.balance - amount_to_spend

    wallet.spend_cash(amount_to_spend)
...
def test_wallet_spend_cash_raises_exception_on_insufficient_amount(empty_wallet):
    ...
    amount_to_spend = 15.0

    with pytest.raises(InsufficientAmount):
        empty_wallet.spend_cash(amount_to_spend)
